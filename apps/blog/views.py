...
from blog.forms import SelectPostForm, SubmitBlogpostForm
...
def index(request):
    ...
    # Make list a paginator object to handle number of posts shown per page
    ...
    return render(request, "blog/index.html", {"blogs": blogs, "current_tags": tags})
...
def select_post(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(
                reverse("blogs:change_post", kwargs={"slug": post.slug})
            )
    else:
...
def change_post(request, slug):
    ...
    if not post.is_published:
        raise Http404(
            "Post is unpublished. First publish the blogpost, then edit the blogpost on the website. "
        )
...
def post_detail(request, slug):
    ...
    teaser80 = (
        post.teaser[0:80]
        + "... | FairBlogs: Vind duurzame en eerlijke blogs via één website!"
    )
    if not post.is_published:
        raise Http404(
            "Post is unpublished. First publish the blogpost, then view the blogpost on the website. "
        )
    return render(
        request,
        "blog/detail.html",
        {"post": post, "full_url": full_url, "teaser80": teaser80},
    )
...
def update_post_counter(request, slug):
    ...
    if not post.is_published:
        raise Http404(
            "Post is unpublished. First publish the blogpost, then use the outlink."
        )
    post.popularity += 1
    post.save()
