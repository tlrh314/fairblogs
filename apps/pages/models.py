...
def validate_only_one_instance(obj):
    ...
    if model.objects.count() > 0 and obj.id != model.objects.get().id:
        raise ValidationError(
            "Errror: only 1 instance of {0} is allowed and it already exists.".format(
                model.__name__
            )
        )
...
class AboutUs(models.Model):
    ...
    logo_projectcece = models.ImageField(
        upload_to=get_company_logo, blank=True, null=True
    )
